...
import { ISuggestedAction } from '../../../common/types/chat_saved_object_attributes';
import { ChatContext, ISuggestionProvider } from './types';

/**
 * Registry for managing custom suggestion providers
 */
export class SuggestionRegistry {
  private providers: Map<string, ISuggestionProvider> = new Map();
   ...
   */
  register(provider: ISuggestionProvider): void {
   ...
   */
  async getCustomSuggestions(context: ChatContext): Promise<ISuggestedAction[]> {
    ...
    const allSuggestions: Array<ISuggestedAction & { providerId?: string; priority?: number }> = [];
